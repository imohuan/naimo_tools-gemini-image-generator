<template>
  ...
  >
    ...
    >
      <!-- 标题栏 -->
      <div
        class="px-6 py-5 border-b border-slate-200 flex items-center justify-between bg-white rounded-t-xl"
      >
        <div>
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div class="flex items-center mb-3">
          <nav class="inline-flex items-center gap-1 p-1 bg-slate-100 rounded-lg">
            <button
        ...
        <div v-if="activeTab === 'basic'" class="space-y-4">
          ...
          <div>
            ...
            <div class="flex gap-2">
              ...
              >
                ...
                <option v-else-if="imageModels.length === 0" value="">
                  暂无可用模型
                </option>
          ...
          <div>
            ...
            <div class="flex gap-2">
              ...
              >
                ...
                <option
                  v-for="model in storyboardModels"
                  :key="model.id"
                  :value="model.id"
                >
                  {{ model.id }}
          ...
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">
              选择生成图片的分辨率，分辨率越高质量越好但消耗更多 token
            </p>
          </div>
        </div>
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-sm font-semibold text-slate-900">提示词模板</h3>
              <p class="text-xs text-slate-500 mt-1">
                修改用于分析图片、生成分镜与最终图片的系统提示词，可根据需要微调。
              </p>
            </div>
            <button
              class="px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-all"
              type="button"
          ...
          <div class="space-y-3">
            <div>
              ...
              <textarea
                v-model="localPrompts.finalPrompt"
                class="w-full min-h-[240px] text-xs px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white resize-y leading-relaxed"
                spellcheck="false"
              />
            </div>
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-700 mb-1">
                图片分析提示词 (imageAnalysis)
              </label>
              <textarea
                v-model="localPrompts.imageAnalysis"
                class="w-full min-h-[240px] text-xs px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white resize-y leading-relaxed"
                spellcheck="false"
              />
            </div>
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-700 mb-1">
                单个分镜提示词模板 (singleStoryboard)
              </label>
              <textarea
                v-model="localPrompts.singleStoryboard"
                class="w-full min-h-[240px] text-xs px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white resize-y leading-relaxed"
                spellcheck="false"
              />
            </div>
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-700 mb-1">
                批量分镜提示词模板 (batchStoryboard)
              </label>
              <textarea
                v-model="localPrompts.batchStoryboard"
                class="w-full min-h-[240px] text-xs px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white resize-y leading-relaxed"
                spellcheck="false"
              />
            </div>
...
import { useAppStore, type PromptTemplates } from "../stores/appStore";
...
const { prompts } = storeToRefs(appStore);

const activeTab = ref<"basic" | "prompts">("basic");
...
const localPrompts = ref<PromptTemplates>({
  ...prompts.value,
});
...
function handleResetPrompts() {
  appStore.resetPromptsToDefault();
  localPrompts.value = {
    ...prompts.value,
  };
}
...
function handleSave() {
  ...
  appStore.updatePrompt({ ...localPrompts.value });
  emit("close");
}
</script>
